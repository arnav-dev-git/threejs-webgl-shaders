...
float circle(vec2 radius, vec2 p){
  return 1.0 - smoothstep(length(radius), length(radius) + 0.008, length(p));
}
...
void main(){
  // float n = cnoise(v_position.xy/10., v_position.xy/20.); 
  float n = srnoise(v_position.xy  * 20., u_time); 
  // float ring = u_Contrast - fract(5.3  * n * sin(u_time));
  // float lerp = pow(ring, u_RingScale) + n;

  vec3 color = mix(vec3(0.0), vec3(1.0), float(n));
