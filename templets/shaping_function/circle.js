...
const fShader = `
  ...
  void main()
  {
    ...
    vec3 g_color = mix(vec3(1.0,0.0,0.0),vec3(1.0,1.0,0.0),uv.x);

    float inCircle = 1.0 - smoothstep(0.5, 0.5067, length(v_position.xy));  //smooth circle  
    // float inCircle = 1.0 - step(0.5, length(v_position.xy));   // rough circle

    // vec3 color = vec3(1.0,1.0,0.0) * inCircle; // yellow color
    vec3 color = g_color * inCircle; // yellow orange blended color
