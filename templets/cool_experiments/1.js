...
float circle(float radius, vec2 p){
  return 1.0 - smoothstep(radius, radius + 0.008, length(p));
}
...
float rect(float a, float b, vec2 p){
  float horz = smoothstep(-a, -a+0.01, p.x) - smoothstep(a, a+0.01, p.x);
  float vert = smoothstep(-b, -b+0.01, p.y) - smoothstep(b, b+0.01, p.y);
  return horz * vert;
}

void main(){
  float n = snoise(v_position); 
  float ring = u_Contrast - fract(5.3  * n * sin(u_time));
  float lerp = pow(ring, u_RingScale) + n;

  mat2 mat = rotationMatrix(u_time);
  mat2 scale = scaleMatrix( sin(u_time * 1.4) * 1.0);
  
  vec2 pt = scale * mat * v_position.xy;

  vec3 color = mix(u_DarkColor, u_LightColor, lerp) * circle(pt.x+pt.y, v_position.xy);
  // vec3 color = mix(u_DarkColor, u_LightColor, lerp) * rect(0.2, 0.2, pt) ;
  // vec3 color = mix(u_DarkColor, u_LightColor, lerp) * (rect(0.2, 0.2, pt) - circle(0.12, v_position.xy));
